                      ...
                      int nHeight = 0, int nConfirmations = 0, int nBlockTime = 0)
{
    ...
    entry.push_back(Pair("size", (int)::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION)));
    entry.push_back(Pair("vsize", (GetTransactionWeight(tx) + WITNESS_SCALE_FACTOR - 1) / WITNESS_SCALE_FACTOR));
    entry.push_back(Pair("version", tx.nVersion));
    entry.push_back(Pair("locktime", (int64_t)tx.nLockTime));
    entry.push_back(Pair("tx-comment", tx.strTxComment));
    UniValue vin(UniValue::VARR);
        ...
        if (tx.IsCoinBase())
            ...
            if (GetSpentIndex(spentKey, spentInfo)) {
                ...
                if (spentInfo.addressType == 1) {
                    ...
                    in.push_back(Pair("address", CBitcoinAddress(CKeyID(uint160(addressBytes))).ToString()));
                } else if (spentInfo.addressType == 2)  {
                    std::vector<unsigned char> addressBytes(spentInfo.addressHash.begin(), spentInfo.addressHash.begin() + 20);
                    in.push_back(Pair("address", CBitcoinAddress(CScriptID(uint160(addressBytes))).ToString()));
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric or string, required) The key is the trezarcoin address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
