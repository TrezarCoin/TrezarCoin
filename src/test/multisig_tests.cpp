...
BOOST_AUTO_TEST_CASE(multisig_verify) {
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        keys += key[i];
        s = sign_multisig(a_and_b, keys, txTo[0], 0);
        BOOST_CHECK_MESSAGE(!VerifyScript(s, a_and_b, txTo[0], 0, flags, 0), \
          strprintf("a&b 1: %d", i));
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (i == 0 || i == 1)
          BOOST_CHECK_MESSAGE(VerifyScript(s, a_or_b, txTo[1], 0, flags, 0), \
            strprintf("a|b: %d", i));
        else
          BOOST_CHECK_MESSAGE(!VerifyScript(s, a_or_b, txTo[1], 0, flags, 0), \
            strprintf("a|b: %d", i));
    ...
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
        {
            ...
            if (i < j && i < 3 && j < 3)
              BOOST_CHECK_MESSAGE(VerifyScript(s, escrow, txTo[2], 0, flags, 0), \
